...
export default function MinhasDenuncias() {
  const [modalDenunciaOrdenar, setModalDenunciaOrdenar] =
    React.useState<boolean>(false);
  const [modalDenunciaFiltrar, setModalDenunciaFiltrar] =
    React.useState<boolean>(false);
  ...
  const handleCardPress = (denuncia: DenunciaData) => {
    if (tipoUsuario === "servidorPublico") {
      router.push({
        pathname: "/atualizarDenuncia",
        params: { id: denuncia._id },
      });
    } else {
      router.push({
        pathname: "/editarDenuncia",
        params: { id: denuncia._id },
      });
    }
  };
  ...
  const handleFilter = async (filters: {
    ...
    try {
      ...
      if (filters.categorias.length > 0)
        params.append("categoria", filters.categorias.join(","));
      if (filters.status.length > 0)
        params.append("status", filters.status.join(","));
      if (filters.startDate)
        params.append("dataInicio", filters.startDate.toISOString());
      if (filters.endDate)
        params.append("dataFim", filters.endDate.toISOString());
      ...
      if (response.ok) {
        ...
        alert(data.message || "Erro ao filtrar denúncias");
      }
    } catch (error) {
  ...
  return (
    <View style={style.container}>
      <NavBar
        title={
          tipoUsuario === "servidorPublico" ? "Denúncias" : "Minhas denúncias"
        }
      />
      <ScrollView>
        <View style={style.buttonView}>
          <TouchableOpacity
            onPress={() => setModalDenunciaOrdenar(true)}
            style={style.buttons}
          >
            <Text style={style.textButton}>Ordenar</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => setModalDenunciaFiltrar(true)}
            style={style.buttons}
          >
        ...
        <View style={style.cardView}>
          {denuncias.length > 0 ? (
            denuncias.map((denuncia, index) => (
              <TouchableOpacity
                key={index}
                onPress={() => handleCardPress(denuncia)}
              >
                <Card>
                  ...
                  <View style={style.endCard}>
                    ...
                    <Text
                      style={[
                        style.status,
                        { backgroundColor: getStatusColor(denuncia.status) },
                      ]}
                    >
                      {denuncia.status}
                    </Text>
