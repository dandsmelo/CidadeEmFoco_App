      ...
      if (response.ok) {
        if (data.needVerification) {
          try {
            ...
            console.error("Erro ao enviar SMS Firebase:", err);
            showMessage("Erro ao enviar SMS de verificação.", "error");
          }
        } else {
          ...
          onPress={async () => {
            ...
            try {
              const response = await fetch(
                "http://localhost:3000/usuario/enviarCodigoRedefinirSenha",
                {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
